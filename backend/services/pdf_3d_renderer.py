...
def render_pdf_3d(x: float, y: float, z: float, output_path: str, step_analysis: Dict[str, Any]) -> bool:
    """PDF verilerinden 3D render oluştur"""
    try:
        print(f"[DEBUG] PDF 3D render: X={x}, Y={y}, Z={z} mm")
        
        fig = plt.figure(figsize=(10, 8), dpi=150)
        ax = fig.add_subplot(111, projection='3d')
        
        # Silindirik geometri kontrolü - daha akıllı mantık
        cyl_diameter = step_analysis.get("Silindirik Çap (mm)")
        cyl_height = step_analysis.get("Silindirik Yükseklik (mm)")
        
        print(f"[DEBUG] Analiz verileri: Silindirik Çap={cyl_diameter}, Yükseklik={cyl_height}")
        print(f"[DEBUG] X vs Y farkı: {abs(x-y)}, Max boyutun %{abs(x-y)/max(x,y)*100:.1f}'i")
        
        # Silindirik geometri tespiti:
        # 1. X ve Y yaklaşık eşit mi? (fark %15'ten az)
        # 2. Silindirik çap mevcut mu ve mantıklı mı?
        is_cylindrical = False
        
        if cyl_diameter and cyl_height:
            # X ve Y'den büyük olanı çap olarak kabul et
            estimated_diameter = max(x, y)
            diameter_diff = abs(cyl_diameter - estimated_diameter) / estimated_diameter
            xy_diff = abs(x - y) / max(x, y)
            
            print(f"[DEBUG] Silindirik çap vs tahmin edilen çap farkı: %{diameter_diff*100:.1f}")
            print(f"[DEBUG] X-Y farkı: %{xy_diff*100:.1f}")
            
            # Eğer X-Y arasında büyük fark varsa muhtemelen prizmatik
            if xy_diff > 0.3:  # %30'dan fazla fark
                is_cylindrical = False
                print(f"[DEBUG] Prizmatik geometri: X-Y farkı çok büyük (%{xy_diff*100:.1f})")
            # Silindirik çap mantıklıysa silindir
            elif diameter_diff < 0.3:  # %30'dan az fark
                is_cylindrical = True
                print(f"[DEBUG] Silindirik geometri tespit edildi")
            else:
                is_cylindrical = False
                print(f"[DEBUG] Prizmatik geometri: Çap uyuşmazlığı")
        
        if is_cylindrical:
            # Silindir çiz - gerçek çapı kullan
            render_cylinder(ax, cyl_diameter/2, cyl_height)
            title = f'PDF Analizi - 3D Silindirik Model\nÇap: {cyl_diameter:.1f}mm, Yükseklik: {cyl_height:.1f}mm'
        else:
            # Dikdörtgen prizma çiz
            render_box(ax, x, y, z)
            title = f'PDF Analizi - 3D Prizmatik Model\n{x:.1f}x{y:.1f}x{z:.1f} mm'
        ...
        print(f"[SUCCESS] PDF 3D render kaydedildi: {output_path}")
        return True
